...
public class DialogCommand {
    ...
    public static void register(CommandDispatcher<CommandSourceStack> p_138061_, CommandBuildContext p_214450_) {
        LiteralCommandNode<CommandSourceStack> literalcommandnode = p_138061_.register(
                Commands.literal("bagus_lib").requires(p_136627_ -> p_136627_.hasPermission(2)).then(Commands.literal("dialog")
                        .then(
                                Commands.argument("targets", EntityArgument.players()).then(Commands.argument("dialog_type", ResourceKeyArgument.key(DialogDataRegister.REGISTRY_KEY)).executes(p_248155_ -> {

                                            Collection<ServerPlayer> collection = EntityArgument.getPlayers(p_248155_, "targets");

                                            if (!collection.isEmpty()) {
                                                sendDialogMessage(p_248155_.getSource(), collection, getDialogType(p_248155_, "dialog_type"));
                                            }
    ...
    public static Holder.Reference<DialogType> getDialogType(CommandContext<CommandSourceStack> p_249310_, String p_250729_) throws CommandSyntaxException {
        return resolveKey(p_249310_, p_250729_, DialogDataRegister.REGISTRY_KEY, ERROR_INVALID_FEATURE);
    }
    ...
    private static <T> ResourceKey<DialogType> getRegistryKey(
            CommandContext<CommandSourceStack> p_212374_, String p_212375_, ResourceKey<Registry<DialogType>> p_212376_, DynamicCommandExceptionType p_212377_
    ) throws CommandSyntaxException {
        ResourceKey<?> resourcekey = p_212374_.getArgument(p_212375_, ResourceKey.class);
        Optional<ResourceKey<DialogType>> optional = resourcekey.cast(p_212376_);
        return optional.orElseThrow(() -> p_212377_.create(resourcekey));
    }

    private static <T> Holder.Reference<DialogType> resolveKey(
