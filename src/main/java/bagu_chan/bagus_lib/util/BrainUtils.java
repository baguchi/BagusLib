...
public class BrainUtils {
    ...
    public static boolean isEntityTargetable(LivingEntity p_26804_, LivingEntity p_26805_, int range) {
        return p_26804_.getBrain().isMemoryValue(MemoryModuleType.ATTACK_TARGET, p_26805_) ? TARGET_CONDITIONS_IGNORE_INVISIBILITY_TESTING.range(range).test(p_26804_, p_26805_) : TARGET_CONDITIONS.range(range).test(p_26804_, p_26805_);
    }

    public static boolean isEntityAttackable(LivingEntity p_148313_, LivingEntity p_148314_, int range) {
        return p_148313_.getBrain().isMemoryValue(MemoryModuleType.ATTACK_TARGET, p_148314_) ? ATTACK_TARGET_CONDITIONS_IGNORE_INVISIBILITY_TESTING.range(range).test(p_148313_, p_148314_) : ATTACK_TARGET_CONDITIONS.range(range).test(p_148313_, p_148314_);
    }
