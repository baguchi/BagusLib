...
public class ClientTestEventHandler {
    @SubscribeEvent
    public static void modelEventAnimation(BagusModelEvent.PostAnimate event) {
        if (event.getModel() instanceof HumanoidModel<?>) {
            TestAnimations.test.bake(event.getModel().root()).apply(event.getBaguAnimationController().getAnimationState(CommonEvent.PAT), event.getEntityRenderState().ageInTicks);
        }
    }
